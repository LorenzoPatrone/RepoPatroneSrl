...
<html>
	...
	<%
		...
		if (category != null && (category.equals("Castagne")) || category.equals("Tartufi") || category.equals("Funghi") || category.equals("Origano")) {
			product = pdao.getByCategory(category);
		} else {
			int price = Integer.parseInt(category);
			product = pdao.getByPrice(price);
		}
	%>
	<body>
		...
		<div class="container">
			...
			<% } %>
			<div class="row">
			<form id="myform" name="myform" method="POST" action="filteredproductpage.jsp">
				...
				<div class="col-md-3 my-3">
					<div class="card w-100" style="width: 18rem;">
						<img class="card-img-top" src="imgs/jpg/<%=p.getImage()%>"
							alt="Card image cap">
						<div class="card-body">
							<h5 class="card-title"> <%=p.getProductName()%> </h5>
							<h6 class="price"> Prezzo: <%=p.getPrice()%> €</h6>
							<h6 class="category"> Categoria: <%=p.getCategory()%> </h6>
							<div class="mt-3 d-flex justify-content-between">
								<a href="add-to-cart?id=<%=p.getProductID()%>" class="btn btn-info">Modifica</a>
								<a href="remove-products?id=<%=p.getProductID()%>" class="btn btn-danger">Rimuovi</a>
							</div>
						</div>
					</div>
				</div>
